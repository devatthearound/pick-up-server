...
import { IsNotEmpty, IsString, IsNumber, IsOptional, IsBoolean, Matches, MaxLength, MinLength } from 'class-validator';

export class CreateStoreDto {
  @ApiProperty({ example: '커피월드', description: '상점 이름' })
  @IsNotEmpty({ message: '상점 이름은 필수입니다.' })
  @IsString()
  ...
  @ApiProperty({ example: '123-45-67890', description: '사업자 등록 번호' })
  @IsNotEmpty({ message: '사업자 등록 번호는 필수입니다.' })
  @IsString()
  ...
  @ApiProperty({ example: 1, description: '상점 카테고리 ID' })
  @IsNotEmpty({ message: '카테고리는 필수입니다.' })
  @IsNumber()
  categoryId: number;

  @ApiProperty({ example: '서울시 강남구 테헤란로 123', description: '상점 주소' })
  @IsNotEmpty({ message: '주소는 필수입니다.' })
  @IsString()
  @MaxLength(255, { message: '주소는 255자를 초과할 수 없습니다.' })
  address: string;

  @ApiProperty({ example: '7층 701호', description: '상점 상세 주소' })
  @IsNotEmpty({ message: '상세 주소는 필수입니다.' })
  @IsString()
  @MaxLength(100, { message: '상세 주소는 100자를 초과할 수 없습니다.' })
  addressDetail: string;

  @ApiProperty({ example: '02-1234-5678', description: '상점 전화번호' })
